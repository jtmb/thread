...
The `thread-auto-context` skill (`alwaysApply: true`) makes context saving automatic — no setup files required. When an agent connects to a workspace with the skill, it:
...
The skill's frontmatter (`alwaysApply: true`) tells the agent to load it automatically — no `.github/copilot-instructions.md` stub needed.

**Without this skill**, agents only use Thread when you explicitly ask.
