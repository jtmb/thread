...
def test_pool_start_opens_all_connections(temp_db_path):
    """Pool.start() opens max_connections and applies pragmas."""
...
def test_active_count_tracks_connections(pool):
    """active_count reflects the number of threads with acquired connections."""
    conn = pool.get()
...
def test_total_connections_after_start(pool):
    """total_connections equals max_connections after start()."""
    assert pool.total_connections == 10
